      ...
      <div className="h-screen flex font-jakarta overflow-hidden bg-[#ECF1F7] text-[#0B1927] text-[14px] leading-[1.55] antialiased">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 page-scroll">
            ...
            {/* ── Welcome Hero ── */}
            <div
              className="hero-grid relative rounded-[12px] px-[30px] py-7 mb-5 overflow-hidden fade-up-1"
              style={{
                background:
                  "linear-gradient(135deg,#0c2340 0%,#0c3a5e 50%,#0369A1 100%)",
              }}
            >
              <div className="relative z-10 flex items-start justify-between gap-5 flex-wrap">
                ...
                <div>
                  <p
                    className="text-[10px] font-bold tracking-[.12em] uppercase mb-[7px]"
                    style={{ color: "rgba(255,255,255,.45)" }}
                  >
                    First time here · Setup takes 2 minutes
                  </p>
                  <h3 className="font-serif-it text-[24px] text-white mb-1.5 leading-[1.25]">
                    Your intelligence dashboard is ready.
                    <br />
                    It just needs your first product.
                  </h3>
                  <p
                    className="text-[13px] mb-5 max-w-[480px] leading-[1.7]"
                    style={{ color: "rgba(255,255,255,.55)" }}
                  >
                    Add a product and we'll run 6 intelligence modules — buyers,
                    markets, keywords, pricing, competitors, and an email
                    sequence — all in one report.
                  </p>

                  {/* Steps */}
                  <div className="flex gap-2.5 flex-wrap mb-[22px]">
                    {[
                      { n: "1", label: "Add a product" },
                      { n: "2", label: "Engine runs (~45 sec)" },
                      { n: "3", label: "Full opportunity report" },
                    ].map(({ n, label }) => (
                      <div
                        key={n}
                        className="flex items-center gap-2 px-[13px] py-2 rounded-[9px]"
                        style={{
                      ...
                      >
                        <div
                          className="w-[22px] h-[22px] rounded-full flex items-center justify-center text-[10px] font-extrabold text-white shrink-0"
                  ...
                  {/* CTA */}
                  <button
                    className="inline-flex items-center gap-2 px-[22px] py-3 rounded-[9px] text-[14px] font-bold text-white cursor-pointer transition-all duration-150 hover:-translate-y-[1px]"
                    style={{
                      background: "linear-gradient(135deg,#0284C7,#0891B2)",
                      boxShadow: "0 4px 16px rgba(2,132,199,.42)",
                    }}
                    onClick={() => navigate("/discover")}
                  >
                    <Plus size={14} color="#fff" strokeWidth={2.2} />
                    Start Your First Intelligence Run
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-2 items-end">
                  {[
                    {
                      v: "Viewing is always free",
                      l: "Only analysis uses queries",
                    },
                    { v: "Setup complete ✓", l: "Profile personalised" },
                  ].map(({ v, l }) => (
                    <div
                      key={v}
                      className="px-[14px] py-[10px] rounded-[9px] text-right"
                      style={{
                        background: "rgba(255,255,255,.08)",
            ...
            <div className="grid grid-cols-4 gap-3 mb-5">
              {STAT_CARDS.map(({ emoji, bg, label, val, hint }, i) => (
                <div
                  key={label}
                  className={`stat-empty bg-white border-[1.5px] border-[#C8D6E2] rounded-[12px] p-4 relative overflow-hidden fade-up-${i + 2}`}
                  style={{ boxShadow: "0 1px 3px rgba(11,25,39,.06)" }}
                >
                  <div
                    className="w-8 h-8 rounded-[6px] flex items-center justify-center text-[15px] mb-[10px]"
                    style={{ background: bg }}
                  >
                    {emoji}
                  </div>
                  <p className="text-[11px] text-[#506A80] mb-1">{label}</p>
                  <p className="text-[22px] font-extrabold text-[#85A3B5] leading-none mb-[3px]">
                    {val}
                  </p>
                  <p className="text-[10px] italic text-[#85A3B5] mt-[5px]">
                    {hint}
                  </p>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-[14px] mb-5">
              ...
              >
                <div className="flex items-center justify-between px-4 py-[13px] border-b border-[#C8D6E2]">
                  <span className="text-[11px] font-bold uppercase tracking-[.08em] text-[#506A80]">
                    Opportunity Hub
                  </span>
              ...
              {/* Recent Activity */}
              <div
                className="bg-white border-[1.5px] border-[#C8D6E2] rounded-[12px] overflow-hidden fade-up-7"
                style={{ boxShadow: "0 1px 3px rgba(11,25,39,.06)" }}
              >
                <div className="px-4 py-[13px] border-b border-[#C8D6E2]">
                  <span className="text-[11px] font-bold uppercase tracking-[.08em] text-[#506A80]">
                    Recent Activity
                  </span>
                </div>
                <div className="p-4">
                  <div className="flex flex-col items-center justify-center py-7 text-center">
                    ...
                    <p className="text-[12px] text-[#506A80] leading-[1.6] max-w-[220px] mb-[14px]">
                      Your analysis runs, reports, and activity will all show up
                      here as you use Report InShort.
                    </p>
                    <div className="w-full mb-[14px] flex flex-col gap-[7px]">
                      {SKEL_WIDTHS_ACT.map((w, i) => (
                        <ActivitySkeletonRow key={i} widths={w} />
                      ))}
                    </div>
                    <button className="inline-flex items-center gap-[5px] px-4 py-[7px] rounded-[6px] text-[12px] font-bold text-[#0284C7] cursor-pointer border-[1.5px] border-[#BAE6FD] bg-transparent hover:bg-[#EAF6FF] transition-all duration-150">
                      Learn how reports work
                    </button>
                  </div>
                </div>
              </div>
