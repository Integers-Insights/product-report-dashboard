...
const ContactForm = ({ popupOpen, setPopupOpen, title }) => {
    ...
    return (
        <div
            className={`h-[100vh] w-full bg-black/50 fixed -top-[100vh] ${popupOpen ? "top-0" : ""}`}
            onClick={() => setPopupOpen(false)}
        >
            <div
                className={`rounded w-80 sm:w-155 md:w-180 m-auto p-3 h-120 sm:h-150 overflow-auto bg-surface fixed -top-[100%] left-1/2 transform -translate-x-1/2 -translate-y-1/2  ${popupOpen ? "top-1/2" : ""}`}
                onClick={(e) => e.stopPropagation()}
            >
                <button className="border border-gray-500 bg-surface rounded-xs p-0.5 float-right m-2" onClick={() => setPopupOpen(false)}><RxCross2 className="text-24 text-primary cursor-pointer" /></button>
                <div className="mt-4">
                ...
                <div className="mt-7">
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                        ...
                        <div>
                            ...
                            <input type="text" id="contactNumber" className="h-9 w-full border border-gray-200 bg-surface px-1" placeholder="+1123456789" value={contact} onChange={(e) => setContact(e.target.value)} />
                        ...
                        <div ref={roleRef} className="relative w-full">
                            ...
                            <div
                                ...
                                className="border border-gray-200 bg-white px-3 py-1.5 cursor-pointer select-none"
                            >
