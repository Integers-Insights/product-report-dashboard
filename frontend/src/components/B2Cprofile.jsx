...
const MaskedBadge = () => (
  <span className="blur-sm inline-flex items-center gap-1 rounded-2xl px-2 py-0.5 text-xs font-medium whitespace-nowrap">
    Lorem, ipsum dolor.
  </span>
);

const safeMap = (val, fn) =>
  Array.isArray(val) ? val.map(fn) : isMasked(val) ? <MaskedBadge /> : null;
...
const B2Cprofile = ({ b2c }) => {
  return (
    <div>
      ...
      {/* Consumer Segment cards — always visible for trial */}
      <div className="mt-6 grid grid-cols-3 gap-6 bg-white">
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
        ...
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          <p className="text-sm font-medium text-[#5F6368]">GENDER SKEW</p>
          <p className="text-sm font-medium text-[#5F6368]">
            {b2c?.consumer_profile?.consumer_segment?.gender_skew ?? "--"}
          </p>
        </div>
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          <p className="text-sm font-medium text-[#5F6368]">INCOME LEVEL</p>
          <p className="text-sm font-medium text-[#5F6368]">
            {b2c?.consumer_profile?.consumer_segment?.income_bracket ?? "--"}
          </p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-6 bg-white">
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          ...
          <p className="flex flex-wrap gap-1.5">
            {safeMap(
              b2c?.consumer_profile?.consumer_segment?.lifestyle_tags,
        ...
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          ...
          <p className="flex flex-wrap gap-1.5">
            {safeMap(
              b2c?.consumer_profile?.consumer_segment?.purchase_motivation,
      ...
      <div className="mt-2 grid grid-cols-3 gap-6 bg-white">
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          ...
          {isMasked(b2c?.purchase_channels) ? (
            <MaskedBadge />
          ) : (
            <ul className="list-disc ml-6 text-sm">
              {safeMap(b2c?.purchase_channels?.online, (itm, i) => (
                <li key={i}>{itm}</li>
              ))}
            </ul>
          )}
        ...
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          ...
          {isMasked(b2c?.purchase_channels) ? (
            ...
            <ul className="list-disc ml-6 text-sm">
              {safeMap(b2c?.purchase_channels?.offline, (itm, i) => (
                <li key={i}>{itm}</li>
              ))}
            </ul>
          )}
        ...
        <div className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover">
          ...
          {isMasked(b2c?.purchase_channels) ? (
            ...
            <ul className="list-disc ml-6 text-sm">
              {safeMap(b2c?.purchase_channels?.social_commerce, (itm, i) => (
                <li key={i}>{itm}</li>
              ))}
            </ul>
          )}
      ...
      <div className="grid grid-cols-2">
        <div className="mt-2">
          ...
          <p className="flex gap-1.5 mt-1.5">
            {isMasked(b2c?.label_preferences) ? (
              <MaskedBadge />
            ) : (
              safeMap(b2c?.label_preferences?.certifications, (item, i) => (
                <span
                  className="bg-[#F3FFF3] text-[#2E7D32] text-xs font-medium px-2 py-0.5 rounded-2xl"
                  key={i}
                >
        ...
        <div className="mt-2">
          ...
          <p className="flex gap-1.5 mt-1.5">
            {isMasked(b2c?.label_preferences) ? (
              ...
              safeMap(b2c?.label_preferences?.key_claims, (item, i) => (
                <span
                  className="bg-[#EDF9FF] text-[#0284C7] text-xs font-medium px-2 py-0.5 rounded-2xl"
                  key={i}
                >
        ...
        <div>
          ...
          <p className="flex gap-1.5 mt-1.5">
            {isMasked(b2c?.label_preferences) ? (
              ...
              safeMap(b2c?.label_preferences?.preferred_formats, (item, i) => (
                <span
                  className="bg-gray-100 text-[#5F6368] text-xs font-medium px-2 py-0.5 rounded-2xl"
                  key={i}
                >
      ...
      <div className="grid grid-cols-3 gap-6 mt-2">
        {isMasked(b2c?.leading_brands) ? (
          <div className="col-span-3">
            <MaskedBadge />
          </div>
        ) : (
          safeMap(b2c?.leading_brands, (item, i) => (
            <div
              className="border border-[#E6E6E6] p-4 rounded-lg flex flex-col gap-1 card-hover"
              key={i}
            >
